...
- **Gemini Versioning:** *NEVER* change a Gemini version number in code unless explicitly asked to, and never based on your own training data — it is always out of date. Verify the current model IDs against https://ai.google.dev/gemini-api/docs/models first. Model IDs live in `TEXT_MODEL` / `IMAGE_MODEL` in `src/utils/gemini.ts` and in `src/utils/tts.ts`; a bump also needs a matching entry in `src/utils/pricing.ts` and an update to the assertion in `src/utils/gemini.test.ts`. As of 2026-08-30 the models in use are `gemini-3.7-flash` (text), `gemini-3.1-flash-lite-image` (images), and `gemini-3.1-flash-tts-preview` (speech).
