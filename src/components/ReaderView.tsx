...
export function ReaderView({
  ...
  const centeredContext = useMemo(() => {
    ...
    return getCenteredContext(words, currentIndex, 200);
  ...
  const currentFontSize = targetFontSize;
  ...
  return (
    ...
    >
      ...
      {!isPlaying && (
        <header className="w-full px-4 py-4 z-20 flex flex-col items-center landscape:items-start landscape:px-12" onClick={(e) => e.stopPropagation()}>
          <div className="w-full flex justify-between items-center mb-4 landscape:mb-2">
            <button
              className="opacity-40 hover:opacity-100 transition-opacity flex items-center gap-1 text-sm"
      ...
      <div className={`relative flex items-center justify-center w-full ${isPlaying ? '' : 'flex-1 overflow-hidden'} border-t border-b ${isPlaying ? 'my-8' : ''} ${theme === 'bedtime' ? 'border-zinc-900' : 'border-zinc-200 dark:border-zinc-800'}`} style={{ minHeight: isPlaying ? Math.max(120, currentFontSize * 1.5) : '120px' }}>
        {isPlaying ? (
          <>
            {!isChapterBreak && (
              <>
          ...
          <div className={`w-full h-full overflow-y-auto px-8 py-12 ${theme === 'bedtime' ? 'text-stone-500' : 'text-zinc-500 dark:text-zinc-400'}`}>
            <div className="flex flex-wrap justify-center gap-x-1.5 gap-y-3 text-xl landscape:text-base leading-relaxed landscape:leading-snug max-w-4xl mx-auto">
              {centeredContext?.before.map((w, i) => (
                <span key={`b-${i}`} className="opacity-40">{w.text}</span>
              ))}
              <span
                ref={currentWordRef}
                className={`font-bold underline ${rsvpFocusColor} scale-110 px-1 transition-transform duration-300`}
              >
                {centeredContext?.current?.text}
              </span>
              {centeredContext?.after.map((w, i) => (
                <span key={`a-${i}`} className="opacity-40">{w.text}</span>
              ))}
            </div>
          </div>
        )}
      ...
      {/* Chapter/Book Progress bars when playing */}
      {isPlaying && (
        <div
          className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-md px-4 flex flex-col gap-4 pointer-events-none z-50"
        >
          {/* Chapter Progress */}
          <div className="space-y-1">
            <div className="text-[10px] uppercase tracking-tighter opacity-30 font-bold">Chapter Progress</div>
            <div className={`w-full h-1 rounded-sm relative ${theme === 'bedtime' ? 'bg-zinc-900' : 'bg-zinc-200 dark:bg-zinc-800'}`}>
              <div
